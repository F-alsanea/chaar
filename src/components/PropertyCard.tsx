...
export const PropertyCard: React.FC<PropertyCardProps> = ({ property, onClick }) => {
  return (
    <motion.div
      ...
      whileHover={{ y: -8 }}
      className="group bg-white rounded-2xl overflow-hidden border border-slate-100 property-card-shadow cursor-pointer"
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4">
          {property.showPrice !== false ? (
            <div className="px-3 py-1 rounded-lg bg-white/90 backdrop-blur-sm text-indigo-900 font-bold text-lg">
              {property.price.toLocaleString()} <span className="text-xs font-normal">ر.س</span>
            </div>
          ) : (
            <div className="px-3 py-1 rounded-lg bg-white/90 backdrop-blur-sm text-indigo-900 font-bold text-sm">
              السعر عند التواصل
            </div>
          )}
        </div>
      ...
      <div className="p-5">
        ...
        <h3 className="text-lg font-bold text-slate-900 mb-4 line-clamp-1 group-hover:text-indigo-600 transition-colors">
          {property.title}
        </h3>
